...
export default function NavComponent() {
  ...
  return (
    ...
    <Navbar onMouseMove={handleHover} onMouseLeave={handleHover} onToggle={handleNavToggle} expanded={navToggleState} className={`${navStyle.my_nav} ${navToggleState? 'nav-open':''} fixed-top ${navState == 'transparent' && !navToggleState?'':!navToggleState && navState == 'invisible'?navStyle.invisible:navState == 'white' && !navToggleState?navStyle.white_nav:!navToggleState? navStyle.hidden_nav:''}`} expand="lg" color={variables.primaryColorLight}>
      <Container fluid>
      ...
      <Navbar.Brand className={navStyle.mobile_brand} href="/" onClick={()=>handleLinkClick(null)}>
        ...
        <Navbar.Collapse id="basic-navbar-nav" className={`px-2`}>
          <Nav className={["me-auto",navStyle.test]}>
            <Link href="/services" >
                <a onClick={()=>handleLinkClick(0)} className={`nav-link ${navStyle.link} ${activeLink == 0 && navStyle.active}`}>SERVICES</a>
            </Link>
            <Link href="/expertise" className={navStyle.test}>
                <a onClick={()=>handleLinkClick(1)} className={`nav-link ${navStyle.link} ${activeLink == 1 && navStyle.active}`}>EXPERTISE</a>
            </Link>
            <Link href="/clients">
                <a onClick={()=>handleLinkClick(2)} className={`nav-link ${navStyle.link} ${activeLink == 2 && navStyle.active}`}>CLIENTS</a>
            </Link>
          </Nav>
          ...
          <Nav className={["ml-auto",navStyle.test3]}>
            <Link href="/group" >
                <a onClick={()=>handleLinkClick(3)} className={`nav-link ${navStyle.link} ${activeLink == 3 && navStyle.active}`}>GROUP</a>
            </Link>
            <Link href="/news" >
                <a onClick={()=>handleLinkClick(4)} className={`nav-link ${navStyle.link} ${activeLink == 4 && navStyle.active}`}>NEWS</a>
            </Link>
            <Link href="/contact" >
                <a onClick={()=>handleLinkClick(5)} className={`nav-link ${navStyle.link} ${activeLink == 5 && navStyle.active}`}>CONTACT US</a>
            </Link>
          </Nav>
          ...
          <Nav className='mobile mobile-nav'>
            ...
            <Link href="https://eshowroom.maisonpyramide.com" target={"_blank"}>
                <a onClick={()=>handleLinkClick(7)} className={`nav-link ${navStyle.link} ${navStyle.arrow} ${activeLink == 7 && navStyle.active}`}>E SHOWROOM</a>
            </Link>
            <Link href="https://www.instagram.com/maisonpyramide/" target={"_blank"}>
