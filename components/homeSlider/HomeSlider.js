        ...
        _items && _items.length && (
            ...
            {_items.map(item => {
                ...
                <TransitionGroup>
                    {(!slideAnimationState && !backSlideAnimationState) && (
                        <CSSTransition key={2} timeout={1000} classNames={"item"}>
                            <span className={sliderStyle.page_count}>{activeIdx < 10 ? '0' : ''}{activeIdx + 1}</span>
                        </CSSTransition>
                    )}
                </TransitionGroup>
                                        ...
                                        <div className={sliderStyle.text}>
                                            ...
                                            <div style={{ width: '100%' }}>
                                                {_items[activeIdx].attributes?.features && (
                                                    ...
                                                    <Link href={`/clients/${_items[activeIdx].attributes?.features}`}>
                                                        <button>
                                                            <a>
                                                                VIEW PROJECT
                                                            </a>
                        ...
                        <Col md={5}>
                            <div className={sliderStyle.rightPart}>
                                ...
                                <div className={sliderStyle.slider_ctrls}>
                                    <div onClick={prevClick} className={`${sliderStyle.slider_ctrl} ${sliderStyle.slider_ctrl_left}`}>
                                        <span className={sliderStyle.chev_left}></span>
                                    </div>
                                    <div onClick={nextClick} className={`${sliderStyle.slider_ctrl} ${sliderStyle.slider_ctrl_right}`}>
                                ...
                                <div className={sliderStyle.mobile_counter}>
                                    {
                                        _items.map((item, key) => {
                                            ...
                                            return (
                                                <span key={key} className={`${sliderStyle.counter_dot} ${key != activeIdx ? sliderStyle.active : ''}`}></span>
                                            )
                                        })
                                    }
                                </div>
                ...
                <Container fluid className="mobile">
                    <Row className={sliderStyle.slider_row} onClick={() => { handleClick(_items[activeIdx]?.attributes?.features) }}>
                        <Col md={5}>
                            <TransitionGroup>
                                {!showMiddleAnimation && (
                                    <CSSTransition key={2} timeout={1000} classNames={"item"}>
                        ...
                        <Col md={5}>
                            <div className={sliderStyle.rightPart}>
                                <div className={sliderStyle.slider_ctrls}>
                                    <div onClick={(e) => prevClick(e)} className={`${sliderStyle.slider_ctrl} ${sliderStyle.slider_ctrl_left}`}>
                                        <span className={sliderStyle.chev_left}></span>
                                    </div>
                                    <div onClick={(e) => nextClick(e)} className={`${sliderStyle.slider_ctrl} ${sliderStyle.slider_ctrl_right}`}>
                                        <span className={sliderStyle.chev_right}></span>
                                    </div>
                                </div>
